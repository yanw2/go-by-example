...
func main() {
	...
	// This also finds the first match but returns the start and end indexes for the match instead of
	// the matching text.
	fmt.Println(r.FindStringIndex("peach punch"))

	// The `Submatch` variants include information about both the whole-pattern matches and the submatches
	// within those matches. For example, this will return information for both `p([a-z]+)ch` and
	// `([a-z]+)`
	...
	// When creating global variables with regular expressions, you can use the `MustCompile` variation of
	// `Compile`. `MustCompile` panics instead of returning an error, which makes it safer to use global
	// variables.
